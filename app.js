...
mongoose.connect('mongodb://127.0.0.1:27017/test'); //mongodb:// 表示链接mongodb的协议
...
/*登录保存状态*/
app.use(session({
    secret:"test",//用来生成session 表示的 字符串
//如果当中这个登陆用户 在60000以内不发任何请求 这个session就失效
    maxAge:60000
...
app.post('/api/delete',function(req,res){
    var id = req.body._id;    
    Comment.findOneAndRemove({_id:id},function(err,doc){
        if(err){
            return
        };
